$(document).ready(function () {
  ...
  if (deleteButtons) {
   ...
   if (deleteConfirmBtn) {
     deleteConfirmBtn.addEventListener('click', () => {
       ...
       deletePopup.classList.add('hidden');
       // Optional: Remove the list item from DOM
       const listItem = document.querySelector('[data-target="list"]');
       if (listItem) {
         listItem.remove();
       }
     });
 ...
 if (saveListBtn) {
       ...
       const newList = `
                <div data-target="list" class="links_ cursor-pointer list-items w-full bg-[#F4F4F4] border border-[#E3E3E3] flex justify-between items-center rounded-xl p-4 my-4">
                  <div class="flex w-full justify-between">
                    ...
                    <div class="flex justify-start w-1/3 text-start items-center gap-2">
                      <p class="text-[#1A1A1A] font-medium">أنت فقط</p>
                      <button class="text-[#8C8C8C]">تعديل</button>
                      <span>|</span>
                      <button class="text-[#FF816C]">حذف</button>
                    </div>
                  </div>
                </button>
       `;
  ...
  $(document).click(function (event) {
    if (!$(event.target).closest('.searchInput, .searchPopup').length) {
      $('.searchPopup').addClass('hidden');
    }
  });
  $('.profileLink').click(function (e) {
    e.preventDefault()
    window.location = 'file:///C:/Users/AL/Desktop/fresh%20market/src/pages/website/sittings.html';
  })
  ...
  $('.saveProductLink').click(function (e) {
    e.preventDefault()
    window.location = 'file:///C:/Users/AL/Desktop/fresh%20market/src/pages/website/sittings.html';
    $(".content-section").addClass("hidden");
    $("#saved-products").removeClass("hidden");
  })
  $('.addreesLink').click(function (e) {
    e.preventDefault()
    var link = window.location = 'file:///C:/Users/AL/Desktop/fresh%20market/src/pages/website/sittings.html';
    if (link) {
      $(".content-section").addClass("hidden");
      $("#address").removeClass("hidden");
    }
  })
  $('.myWalletLink').click(function (e) {
    e.preventDefault()
    var link = window.location = 'file:///C:/Users/AL/Desktop/fresh%20market/src/pages/website/sittings.html';
    if (link) {
      $(".content-section").addClass("hidden");
  ...
  // multi select
  var mselect = document.querySelectorAll(".multi-select");
  if (mselect) {
    mselect.forEach((el) => {
      new TomSelect(el, {
  ...
  setTimeout(() => {
    document.getElementById("loading-shimmer").style.display = "none";
    document.querySelector(".swiper").classList.remove("hidden");
  }, 2000); // تقدر تتحكم في الوقت
  $('.elevator').hide()
